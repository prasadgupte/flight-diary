// Lucide SVG icon component — icon data from https://lucide.dev (v0.460)
// Each icon is [children] where child = ["tag", {attrs}] or ["tag", {attrs}, [grandchildren]]
...
// Convert Lucide child descriptor to React element
function _lucideChild(child, idx) {
  const [tag, attrs, children] = child;
  // Convert SVG attribute names for React (kebab → camelCase)
  const reactAttrs = { key: idx };
  for (const [k, v] of Object.entries(attrs)) {
    const camel = k.replace(/-([a-z])/g, (_, c) => c.toUpperCase());
    reactAttrs[camel] = v;
  }
  if (children && children.length) {
    return React.createElement(tag, reactAttrs, ...children.map(_lucideChild));
  }
 ...
 * @param {string} name — icon key from LUCIDE_ICONS (e.g. "plane", "map-pin")
 * @param {number} size — width & height in px (default 16)
 * @param {string} color — stroke color (default "currentColor")
 * @param {number} strokeWidth — SVG strokeWidth (default 2)
 * @param {string} className — optional CSS class
 * @param {object} style — optional inline styles
 */
function LucideIcon({ name, size = 16, color, strokeWidth = 2, className = "", style = {} }) {
  const children = LUCIDE_ICONS[name];
